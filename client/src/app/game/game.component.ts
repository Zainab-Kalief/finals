...
export class GameComponent implements OnInit {
 ...
 answers: object
    ...
    this._service.all_games()
      .then(data => {
        if(data.status) {
          ...
          for(var i=0; i<3; i++) {
            this.random_games.push(this.all_games[i])
          }
        }
      })
  this.answers = {
  ...
  post_answers() {
    var game1_id = this.random_games[0]._id
    var game2_id = this.random_games[1]._id
    var game3_id = this.random_games[2]._id
    this._service.user_score(this.answers, game1_id, game2_id, game3_id)
      .then(data => {
        ...
        this._router.navigate([`/home/Your score is ${data.data.scores[data.data.scores.length-1]} out of 3 (${percent}%)`])
      })
